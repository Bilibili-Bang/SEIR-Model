...
to received-and-cured
  ...
  if current-time >= (receive-cure-response-time + latent-time)[
    ...
    let num count has-infected-not-cure
    ;;num是未治愈患者数量
    if num > 0 [
      ask has-infected-not-cure [
        recovery  ;;继续治疗
      ]
    ]
  ]
...
to move-turltes
  ifelse human-flow-range = 0[
    fd 0 ;;不移动
  ][
  ;;根据human-flow-range值设置活动范围
...
"" ""
PENS
"susceptible" 1.0 0 -10899396 true "" "plot count turtles with [state = 0]"
"exposed" 1.0 0 -1184463 true "" "plot count turtles with [state = 1]"
